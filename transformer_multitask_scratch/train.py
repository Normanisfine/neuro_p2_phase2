"""
Transformer Multitask Scratch — Phase 2
=======================================

Trains a decoder from scratch with:
  - joint position + velocity prediction
  - consistency loss between d(position) and velocity
  - synthetic extra channel dropout during training
  - balanced easy+hard validation
  - overlap-averaged hybrid inference

This follows the "train from scratch end-to-end" direction from the Phase 2 notes.
"""
...
WANDB_PROJECT = "neuro-p2-scratch-mt"
MAX_SEQ_LEN = 192
INPUT_DIM = 96 + 96
N_VEL = 2
...
class TransformerMultitaskDecoder(nn.Module):
    def __init__(self, input_dim=INPUT_DIM, d_model=256, nhead=4, num_layers=6, d_ff=512, dropout=0.1):
        super().__init__()
...
class TrialDataset(Dataset):
    def __init__(self, session_ids, data_dir, max_seq_len=MAX_SEQ_LEN, extra_dropout_max=8, training=True):
    ...
    def __getitem__(self, idx):
        ...
        feat = np.concatenate([sbp_chunk, d_chunk], axis=1)
...
def validate_model(model, val_ids, data_dir, device, max_seq_len, stride, position_blend):
    ...
    for sid in val_ids:
        sess = load_session(data_dir, sid, is_test=False)
        pred_pos = predict_session_positions(model, sess, device, max_seq_len, stride, position_blend)
...
def main():
    ...
    pa.add_argument("--num-layers", type=int, default=6)
    ...
    pa.add_argument("--position-blend", type=float, default=0.7)
    ...
    if use_wandb:
        ...
        name = args.wandb_name or f"txf_mt_scratch_s{args.seed}_L{args.num_layers}"
    ...
    model = TransformerMultitaskDecoder(
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"| val_r2={vr2:.4f} {'*' if is_best else ''} | lr={lr:.2e} | {dt:.0f}s"
        )
        if use_wandb:
            wandb.log(
                {
