...
Key features:
  - Long sliding-window training (default 10s / 500 bins, 75% overlap)
...
WINDOW_SIZE   = 500   # default 10 s at 50 Hz
WINDOW_STRIDE = 125   # 75% overlap (stride = 25% of window)
...
class SlidingWindowDataset(Dataset):
    ...
    Windows of `window_size` bins (default 10 s = 500 bins at 50 Hz) are
    extracted with `window_stride` step (default 125 = 75% overlap) from
